function [ SAMPLES,OBSERVATIONS,MULTIPLICITY, G_approx, log_approx, evaluated_u_mh,evaluated_Gu_mh, TIMES, W, CUM_ITER, BASISSIZE ]...
    = DAMH_dcg( u,y,sp,noiseInvCov,priorInvCov,mu0,propL,N,period_saving,time_start,maxtime,batch,label,G_approx)
...
%             G_approx=upgrade_approx(evaluated_u_mh,evaluated_Gu_mh,ones(size(evaluated_u_mh,1),1));
%         else
%             WW=W;
%             if size(W,2)>20
%                 WW=W(:,end-20+1:end);
...
%         end
    ...
    if counter_save==period_saving
        ...
        if mod(l/period_saving,2)==0
            save(['res/DAMH_' label '_0.mat'],'SAMPLES','OBSERVATIONS',...
                'MULTIPLICITY','RATES','TIMES','APERTURE',...
                'prijato','neprijato','nepredprijato','-v7.3');
            save(['res/DAMHdata_' label '_0.mat'],'evaluated_u_mh','evaluated_Gu_mh','-v7.3');
        else
            save(['res/DAMH_' label '_1.mat'],'SAMPLES','OBSERVATIONS',...
                'MULTIPLICITY','RATES','TIMES','APERTURE',...
                'prijato','neprijato','nepredprijato','-v7.3');
            save(['res/DAMHdata_' label '_1.mat'],'evaluated_u_mh','evaluated_Gu_mh','-v7.3');
        end
...
save(['res/INFO_DAMH_' label '.mat'],'propL','neprijato','prijato','nepredprijato');%,'LENG','PSTD','ARAT','NOOG','CORR','EFFI')
% figure; imagesc(W)
