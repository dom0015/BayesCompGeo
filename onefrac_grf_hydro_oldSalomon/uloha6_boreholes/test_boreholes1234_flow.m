...
mu0=[-7 -7]';       % prior mean
sigma=1e-11*ones(4*n_configs,1);
%sigma(pattern)=5e3;
...
G=@(ln_d)observation_wells_flow( A,b_,freeNode_,u0_,flux_obtain_matrix,pressure_obtain_matrix,fracture_matrice,intersections,alfa_inter,lengths,node,elem,h_elem,p,mat_frac,chosen_confs,ln_d );
...
time_start=tic;
label=['boreholes1234_flow' '_' num2str(s) '_' num2str(length(G_real)) '_' num2str(steps_MH_approx)];
if approx_type==0
    label = [label '_rbf'];
else
    label = [label '_col'];
