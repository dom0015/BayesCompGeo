...
chosen_confs = [3 4]; n_configs = length(chosen_confs);
...
steps_MH = 200;
steps_MH_approx=2000;         % number of all MH steps
period_saving=1000;
sigmaMH=1.5*gamma*ones(s,1);    % standard deviation of the instrumental density
approx_type=1;      % approximation type - 0 rbf, 1 col

%% observation operator G
[ A,b_,freeNode_,u0_,flux_obtain_matrix,pressure_obtain_matrix,fracture_matrice,intersections,alfa_inter,lengths,node,elem,h_elem,p,mat_frac ] = preparation_wells();
G=@(ln_d)observation_wells( A,b_,freeNode_,u0_,flux_obtain_matrix,pressure_obtain_matrix,fracture_matrice,intersections,alfa_inter,lengths,node,elem,h_elem,p,mat_frac,chosen_confs,ln_d );
...
time_start=tic;
label=['two1' '_' num2str(s) '_' num2str(length(G_real)) '_' num2str(steps_MH_approx)];
if approx_type==0
    label = [label '_rbf'];
else
    label = [label '_col'];
