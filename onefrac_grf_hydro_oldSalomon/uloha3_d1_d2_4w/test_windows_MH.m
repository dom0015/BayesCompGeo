...
chosen_windows = [2 4]; nw = length(chosen_windows);
pattern = logical([0 0 0 1 1 1 0 0 0 1 1 1]);
s=2;                % number of random parameters (length of u)
batch=14;
gamma=2;            % prior standard deviation
mu0=[-7 -7]';       % prior mean
sigma=1e-14*ones(3*nw,1);
%sigma=1e-11*ones(3*nw,1);
%sigma(pattern)=1e-11;
u0=mu0;             % initial iteration of the MH algorithm
steps_MH = 200;
steps_MH_approx=10000;         % number of all MH steps
period_saving=100;
sigmaMH=0.2*ones(s,1);    % standard deviation of the instrumental density
...
[evaluated_u_col,evaluated_Gu_col,evaluated_u_rbf,evaluated_Gu_rbf]=MCMC_standard( u0,y,G,sigma,gamma,mu0,0.1*sigmaMH,steps_MH,period_saving,batch*2,label);
