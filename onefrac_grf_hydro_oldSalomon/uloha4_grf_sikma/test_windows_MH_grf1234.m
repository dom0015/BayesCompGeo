...
n_windows=8; % number of windows on the outflow side
chosen_windows = [1 2 3 4]; n_configs = length(chosen_windows);
...
G=@(x)observation_windows( A,b,freeNode,U0,downEdge,rightEdge,upEdge,leftEdge,fracture_matrice,intersections,alfa_inter,lengths,node,elem,h_elem,p,mat_frac,chosen_windows,x,data_generator,n_windows );
...
time_start=tic;
label=['grf_sikma1234' '_' num2str(s) '_' num2str(length(G_real)) '_' num2str(steps_MH_approx)];
if approx_type==0
    label = [label '_rbf'];
else
    label = [label '_col'];
