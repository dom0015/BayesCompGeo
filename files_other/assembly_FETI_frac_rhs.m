function [problem_setting] = assembly_FETI_frac_rhs(problem_setting,frac_press,ugrad)
...
if isempty(ugrad)
    ...
    [sub_boundary,sub_boundary_val,volume_forces]=divide_neumann_boundary_and_force(Neumann_boundaries,N_bound_value,ugrad);
    
    %% Elasticity sub-Matrices assembly ---------------------------------------
    b=cell(sumbdomains_FETI,1);
    for i=1:sumbdomains_FETI
        [b{i}]=elasticity_assembly_neumann_and_force_only(sub_nodes{i},sub_elem{i},sub_boundary{i},sub_boundary_val{i},volume_forces{i});
    end    
